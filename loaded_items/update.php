...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($picked_err) && empty($order_err) && empty($center_err) && empty($emp_err) && empty($route_err) && empty($status_err)){
        ...
        $sql = "UPDATE loaded_items SET picked_id=?, order_id=?, destination_center_id=?, emp_id=?, route_id=?, status=? WHERE load_id=?";
         
        if($stmt = mysqli_prepare($db, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssssi", $param_picked, $param_order, $param_center, $param_emp, $param_route, $param_status, $param_id);
    ...
    if(isset($_GET["load_id"]) && !empty(trim($_GET["load_id"]))){
        // Get URL parameter
        $id =  trim($_GET["load_id"]);
        
        // Prepare a select statement
        $sql = "SELECT * FROM loaded_items WHERE load_id = ?";
        if($stmt = mysqli_prepare($db, $sql)){
            // Bind variables to the prepared statement as parameters
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <p>Please edit the input values and submit to update the load record.</p>
